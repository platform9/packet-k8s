...
def add_pf9_ostackhost_neutron(url, token, host_id, instances_path, pub_ip):
    body = {
        "instances_path":"{0}".format(instances_path),
        "novncproxy_base_url":"http://{0}:6080/vnc_auto.html".format(pub_ip)
    }
    #import pdb; pdb.set_trace()
    role_add = add_role(url, token, body, host_id, "pf9-ostackhost-neutron")
    return role_add
...
def main():
    ...
    print "Authing pf9-ostackhost-neutron"
    add_pf9_ostackhost_neutron(resmgr_url, token, host_id, "/opt/pf9/data/instances/", pub_ip)
    print "Authing pf9-neutron-base"
    add_pf9_neutron_base(resmgr_url, token, host_id)
    print "Authing pf9-ceilometer"
    add_pf9_ceilometer(resmgr_url, token, host_id, "/opt/pf9/data/instances/")
    print "Authing pf9-neutron-ovs-agent"
